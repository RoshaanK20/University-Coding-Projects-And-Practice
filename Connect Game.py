...
class ConnectGameGUI(QMainWindow):
    def __init__(self, rows=6, columns=7):
        ...
        self.players = []  # Players will be set dynamically
    ...
    def initUI(self):
        ...
        # Number of players and assign markers
        ...
        self.status_label = QLabel(f"Player 1's Turn ({self.players[0]})")
    ...
    def get_player_info(self):
        """Prompt the user to input the number of players and assign markers."""
        while True:
            num_players, ok = QInputDialog.getInt(
                self, "Number of Players", "Enter number of players (2 or 3):", 2, 2, 3
            )
            if ok:
                self.players = ['X', 'O', 'V'][:num_players]  # Assign markers dynamically
                break
        QMessageBox.information(self, "Players Set", f"{num_players} players selected: {', '.join(self.players)}")

    def make_move(self, row, col):
        """Handle player move at a specific cell."""
        # if the cell is valid
        if self.board[row][col] != ' ':
            QMessageBox.warning(self, "Invalid Move", "This cell is already filled!")
            return

        # Rule of game cell (we can start the game from last row)
        if row < self.rows - 1 and self.board[row + 1][col] == ' ':
            QMessageBox.warning(self, "Invalid Move", "You must place on top of a filled cell or in the bottom row!")
            return
        ...
        if self.check_win(self.players[self.current_player]):
            self.highlight_winning_combination(self.players[self.current_player])
        ...
        self.status_label.setText(f"Player {self.current_player + 1}'s Turn ({self.players[self.current_player]})")
        self.highlight_valid_moves()

    def check_win(self, marker):
        """Check for a winning condition."""
        # Checking horizontal vertical and diagonal
        for r in range(self.rows):
    ...
    def highlight_winning_combination(self, marker):
    ...
    def reset_game(self):
        ...
        for r in range(self.rows):
            for c in range(self.columns):
                ...
                self.buttons[r][c].setStyleSheet("")
        self.current_player = 0
        self.status_label.setText(f"Player 1's Turn ({self.players[0]})")
        self.highlight_valid_moves()
